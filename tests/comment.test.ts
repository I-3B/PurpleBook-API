...
describe("comment route", () => {
    describe("addComment", () => {
        test("should be able to comment on a post", async () => {
            ...
            const { comment } = (await addComment(token, postId, 1, 201)).body;
            expect(comment.content).toBe("b");
        });
    ...
    describe("getAllComments", () => {
        ...
        test.only("should return a partition of comments when using skip and limit", async () => {
            const { post } = (await addPost(token, 1, 201)).body;

            for (let i = 1; i <= 30; i++) {
                await addComment(token, post._id, i, 201);
            }

            const { comments } = (
                await getAllComments(token, post._id, 200, "skip=10&limit=5&sort=date")
            ).body;
            //limit = 5
    ...
    describe("editComment", () => {
        test("should be able to edit a comment when authorized", async () => {
            ...
            } = (await addComment(token, postId, 1, 201)).body;
            const { comment } = (await editComment(token, postId, commentId, 3, 200)).body;
            expect(comment.content).toBe("bbb");
        });
